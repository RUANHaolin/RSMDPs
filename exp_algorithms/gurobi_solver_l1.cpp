...
tuple<numvec, vector<int>, prec_t> VI_rmdp_sarect_runtimes(const vector<vector<numvec>> &P, int S, int A, const numvec &r, double gamma, double radius, int max_iter, double tol){
//    auto start_pre  = std::chrono::high_resolution_clock::now();
    ...
    for ( int iter = 0; iter < max_iter; iter++ ){
        ...
        for ( int s = 0; s < S; s++ ){
            double BV_max = -99.0;
            for ( int a = 0; a < A; a++ ){
                auto [ p, runtime_temp ] = sarect_solve_gurobi_l1_rumtimes(S, V, P[s][a], radius);
